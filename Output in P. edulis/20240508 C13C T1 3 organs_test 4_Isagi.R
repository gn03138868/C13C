...
{
  data <- read.csv("C:/Users/Sorbus/Downloads/work log on task 2/T1exp.csv", header = TRUE, sep = ',')
  ...
  k1B <- 0.387716815
  C2B <- 0.03316654
  #C1C <- 1.001632481 here we did not sample after 13C pulse labelling on Day 0 due to concern treatment effect of digging holes in the culm until D14
  #k1C <- 0.334551917
  C2C <- 0.001694612
  
  Total13C <- 6.767228174 + 1.568735239
}
...
initial_guess_culm <- c(0.334551917, 1.001632481, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01)
...
# Set initial guess for parameters
initial_guess <- c(0.01, 0.01, 0.334551917, 0.01)
...
write.csv(df, file = "theoretical_valuesfittoexperimentsT1_R_4_Isagi.csv", row.names = FALSE)
}
...
ggsave("C:/Users/Sorbus/Downloads/work log on task 2/T1_3organs_plot_R4_Isagi.jpg", p, dpi = 600, width = 10, height = 6)
}
